...
| `RAG_RERANK_CMD` | 未设置 | 仅 MCP 服务器：用于重排搜索结果的外部命令。未设置时不进行重排。 |
...
设置 `RAG_RERANK_CMD` 后，外部命令会对 MCP 工具 `query_documents` 的结果重新排序。服务器会把搜索查询和命中片段的文本写入该命令的标准输入，因此调用远程服务的命令会把你的查询和文档内容发送到本机之外。未设置该变量时不会进行重排，CLI 也不会使用它。

该值是命令及其参数，以空格分隔。它不经过 shell 执行，因此命令必须可直接执行：在 Windows 上无法启动由 npm 安装的 `.cmd` 或 `.bat` 包装脚本，需要直接指定可执行文件。服务器会在你配置的参数后追加 `--query <文本>` 和 `--top <数量>`。
...
如果命令无法启动、执行失败、超过 `RAG_RERANK_TIMEOUT_MS`，或返回的内容无法与服务器自身的结果对应，结果将保持原有顺序。
